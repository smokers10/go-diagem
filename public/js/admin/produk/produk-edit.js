$(document).ready(function () {
    ...
    $var1 = $('#field-var1_pilihan');
    $var1.tagsinput({
        tagClass: 'badge badge-primary',
        maxTags: 20
    });

    $($var1).on('itemAdded', function (event) {
        update_variasi();
    });

    $($var1).on('itemRemoved', function (event) {
        var tag = event.item;
        $('#tbl_variasi tbody').find('tr').each(function () {
            $(this).find('td input[value="' + tag + '"]').eq(0).closest('tr').remove();
            variasi_id = $(this).find('td input.variasi_id-'+ tag).val();
            if(variasi_id)
            {
                $('#var2_status').after('<input type="hidden" name="hapus_variasi[]" value="'+variasi_id+'" />')
            }
        });
    });
    ...
    $("#upload").on("click", function () {
        ...
        croppie.result({
            ...
            $("#foto-" + id).val(base64);
            $("#btn-foto-"+id).addClass("d-none");
            $("#btn-hapus-"+id).removeClass("d-none");
        });
    });
    ...
    $("#form-produk").on('submit', function (e) {
        ...
        $.ajax({
            url: laroute.route('admin.produk.update'),
            type: 'POST',
            data: formData,
            cache: false,
            contentType: false,
    ...
    $(".btn-foto_hapus").on("click", function () {
        ...
        $("#previewImg-" + foto_id).attr("src", laroute.url('public/img/placeholder/product.png', ['']));
        $("#btn-foto-"+foto_id).removeClass("d-none");
        $("#btn-hapus-"+foto_id).addClass("d-none");
    });
});
