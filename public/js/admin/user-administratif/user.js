jQuery(document).ready(function () {
    $('#list-user-admin').DataTable({
        "ajax":"/admin/user-administratif/get",
        "columns":[
            {
                "data": null,
                ...
                "render":function(data, type, row){
                    ...
                    <div class="btn-group" role="group">
                        ...
                        <div class="dropdown-menu" aria-labelledby="actionButton-${data.id}">
                            <a class="dropdown-item" href="/admin/user-administratif/edit/${data.id}">
...
function hapus(id) {
    ...
    .then((result) => {
        if (result.value) {
        $.ajax({
            url: "/admin/user-administratif/delete",
            data : {id},
            type: "DELETE",
            dataType: "JSON",
            beforeSend: function(){
            ...
            success: function(response) {
                if (response.success) {
                    Swal.fire({
                        title: "Berhasil",
                        text: 'User Admin Berhasil Dihapus!',
                        timer: 3000,
                        showConfirmButton: false,
                        icon: 'success'
                    })
                    $('#list-user-admin').DataTable().ajax.reload()
                }else {
